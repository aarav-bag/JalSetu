...
const MiniGauge = ({ level, color }: { level: number; color: string }) => {
  ...
  return (
    <svg width="72" height="72" viewBox="0 0 72 72" className="-rotate-90">
      <circle cx="36" cy="36" r={r} stroke="rgba(128,128,128,0.25)" strokeWidth="6" fill="none" />
      <circle
        cx="36" cy="36" r={r}
      ...
      <div className="flex gap-1 h-4 rounded-full overflow-hidden">
        {segments.map((s, i) => (
          <div key={i} className="flex-1 rounded-sm"
            style={{ background: s.color, opacity: i === 1 ? 1 : 0.55 }} />
        ))}
      </div>
...
const STEPS: Step[] = [
        ...
        <div className="grid grid-cols-3 gap-3 w-full">
          {[
            ...
            <div key={m.label}
              ...
              style={{ background: `${m.color}22`, border: `1px solid ${m.color}45` }}>
  ...
  },
  {
    ...
    subtitle: "How wet your soil is — shown as a %",
    description:
      ...
      <div className="w-full grid grid-cols-3 gap-2">
        {[
          ...
          <div key={r.label}
            ...
            <div className="text-[9px] text-gray-400 dark:text-gray-500">{r.range}</div>
  ...
  },
  {
        ...
        {[
          ...
          <div key={s.badge}
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold flex-shrink-0"
              style={{ background: `${s.color}25`, color: s.color, border: `1px solid ${s.color}45` }}>
              <CheckCircle2 className="h-3 w-3" />
              {s.badge}
            </div>
            <span className="text-xs text-gray-600 dark:text-gray-300">{s.meaning}</span>
          </div>
        ))}
    ...
    tip: "You can replay this tour anytime from Settings → Farm Metrics Guide",
  },
];
...
export function OnboardingTour({ onComplete }: OnboardingTourProps) {
  ...
  const cardBg       = darkMode ? "rgba(18,22,34,0.97)"   : "rgba(255,255,255,0.98)";
  const cardShadow   = darkMode
    ? "0 25px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.08)"
    : "0 25px 60px rgba(0,0,0,0.22), 0 0 0 1px rgba(255,255,255,0.7)";
  const visualBg     = darkMode ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)";
  const visualBorder = darkMode ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)";
  const dotInactive  = darkMode ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.15)";
  ...
  const descColor    = darkMode ? "#cbd5e1" : "#475569";
  const backBtnColor = darkMode ? "#94a3b8" : "#64748b";
  const backBtnHover = darkMode ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)";
  const tipBg        = darkMode ? "rgba(59,130,246,0.12)"  : "rgba(59,130,246,0.07)";
  const tipBorder    = darkMode ? "rgba(59,130,246,0.25)"  : "rgba(59,130,246,0.15)";
  const tipColor     = darkMode ? "#93c5fd" : "#1d4ed8";
  const closeBtnHover= darkMode ? "rgba(255,255,255,0.1)"  : "rgba(0,0,0,0.07)";

  return (
    // z-[200] sits above the bottom nav's z-50
    <div
    ...
    >
      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 id="tour-title" className="text-base font-bold leading-tight" style={{ color: titleColor }}>
                {current.title}
              </h2>
            </div>
          </div>
        ...
        <div className="px-5 mb-3 flex-shrink-0">
          <div className="h-1.5 rounded-full overflow-hidden" style={{ background: progressBg }}>
            <motion.div
          ...
          <p className="text-[10px] mt-1 text-right" style={{ color: subtitleColor }}>
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              <div className="rounded-2xl p-4"
                style={{ background: visualBg, border: `1px solid ${visualBorder}` }}>
                {current.visual}
              </div>
              ...
              {current.tip && (
                <div className="flex gap-2 items-start rounded-xl px-3 py-2.5"
                  ...
                  <p className="text-xs leading-snug" style={{ color: tipColor }}>{current.tip}</p>
                </div>
              )}
            </motion.div>
          </AnimatePresence>

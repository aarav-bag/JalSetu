...
export default function PageShell({ children, className = "" }: PageShellProps) {
  return (
    <div className={`max-w-md mx-auto min-h-screen flex flex-col relative transition-colors duration-500 ${className}`}>
      ...
      {/* Orbs in a separate fixed overlay so they don't affect fixed nav positioning */}
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="orb top-[55%] left-[-40px] w-56 h-56 bg-emerald-400 opacity-10 dark:opacity-15" />
        <div className="orb bottom-[-60px] right-[20%] w-72 h-72 bg-indigo-400 opacity-15 dark:opacity-20" />
        <div className="orb bottom-[25%] left-[30%] w-48 h-48 bg-purple-400 opacity-10 dark:opacity-15" />
      </div>

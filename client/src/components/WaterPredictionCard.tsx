...
const WaterPredictionCard = ({ farmId = 1 }: WaterPredictionCardProps) => {
  ...
  return (
    <div className="mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {isSet && location ? (
            <button
              ...
              className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-300 px-2 py-0.5 rounded-full transition-opacity hover:opacity-70"
            ...
            <button
              onClick={() => setShowPicker(v => !v)}
              className="flex items-center gap-1 text-[10px] font-semibold text-blue-600 dark:text-blue-300 px-2 py-0.5 rounded-full transition-opacity hover:opacity-70"
      ...
      {/* Location picker (shown inline when toggled) */}
      {showPicker && (
        <div className="mb-3">
          <LocationPicker onSet={() => { setShowPicker(false); refetch(); }} compact={false} />
        </div>
      )}
      ...
      <div className="glass-card rounded-[1.5rem] p-5">
        ...
        {!isSet && !showPicker && (
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-2xl mb-4 text-left transition-all hover:scale-[1.01]"
            style={{ background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.2)" }}>
            <div className="h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: "rgba(59,130,246,0.18)" }}>
              <MapPin className="h-4.5 w-4.5 text-blue-600 dark:text-blue-300" />
            </div>
            <div>
              <p className="text-sm font-bold card-heading">Enter your location</p>
              <p className="text-xs card-body mt-0.5">Get accurate weather for your farm — tap to set your city or village</p>
            </div>
          </button>
        )}
        ...
        {isLoading && (
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 rounded-2xl glass-tile">
              ...
              <div>
                <p className="text-sm font-semibold card-heading">Fetching weather…</p>
                <p className="text-xs card-muted mt-0.5">Loading your forecast</p>
              </div>
            </div>
